...
treatmentList = list(
  ...
  c("T_oxalic_trickle_", "T_oxalic_trickle_total", "Oxalic acid - tickling"),
...
for(i in treatmentList){
  ...
  treatmentexp <- paste("(", i[1], ")0[1-2]", sep = "")
...
for(i in treatmentList){
  # Get Columns which are starting with List value
  treatmentexp <- paste("(", i[1], ")0[3-7]", sep = "")
  x <- grepl(treatmentexp, colnames(D.FULL), fixed = FALSE, perl = TRUE)
  # sum the row values (means 1 = for 1 month, 2 = 2 months etc.)
...
for(i in treatmentList){
  # Get Columns which are starting with List value
  treatmentexp <- paste("(", i[1], ")0[8-9]|(", i[1], ")1[0]", sep = "")
  x <- grepl(treatmentexp, colnames(D.FULL), fixed = FALSE, perl = TRUE)
  # sum the row values (means 1 = for 1 month, 2 = 2 months etc.)
