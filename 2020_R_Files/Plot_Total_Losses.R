...
D.FULL.PROD <- select(D.FULL.PROD, hives_production, hives_winter, state)
...
rm(temp.df, i, state.list, D.FULL.PROD)
# We use the alpha later for plotting
D.STATES.PROD$alpha <- ifelse(D.STATES.PROD$state == "Österreich", 0.5, 0)
...
p4 <- 
  ggplot( D.STATES.PROD, aes( x = state, y = mean )) +
  geom_bar( aes(alpha = alpha), colour = "black", fill = color_rule, show.legend = FALSE, stat = "identity", linetype = "longdash" ) + 
...
plot_grob <- arrangeGrob( p1, p2, p3,
             #top = gtitle, 
             layout_matrix = lay)
# Save File
#ggsave("./img/Plot_Total_Losses.pdf", p, width = 12, height = 8, units = "in")
ggsave("./img/Plot_Total_Losses.pdf", plot_grob, width = 10, height = 7, units = "in")
