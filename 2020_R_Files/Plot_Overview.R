...
p1 <- ggplot( data = D.SIZE.PLOT ) +
  ...
  scale_y_continuous(
    ...
    limits = c( 0, max(D.SIZE.PLOT$n)+100 )
  )

p2 <- ggplot( data = D.SIZE.PLOT ) +
...
p3 <- ggplot( data = D.LOSSGROUP.PLOT ) +
  ...
  scale_y_continuous(
    ...
    limits = c( 0, max(D.LOSSGROUP.PLOT$n)+150 )
  )
...
plot_grob <- arrangeGrob( p1, p2, p3, p4,
              #top = gtitle, 
              layout_matrix = lay)

# Save File
ggsave("./img/Plot_Overview.pdf", plot_grob, width = 10.5, height = 8, units = "in")
